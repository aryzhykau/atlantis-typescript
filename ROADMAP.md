...
- [ ] **1. Подготовка и Общее Проектирование:**
    ...
    - [ ] **API и Модели Данных:**
        ...
        - [ ] Определить API эндпоинты для получения актуальных тренировок (с детализацией по студентам), также используя `injectEndpoints`.
    ...
    - [ ] **Общие Принципы UI/UX:**
        - [ ] **Стиль:** Соблюдать минималистичный стиль, избегая излишних визуальных элементов.
        - [ ] **Навигация по неделям:** Стрелки "предыдущая/следующая неделя" + `DatePicker` (выбор даты открывает содержащую ее неделю).
        - [ ] **Временная шкала:** Вертикальная, с 06:00 до 22:00, часовые слоты. Тренировки начинаются в XX:00, длительность 1 час.
        - [ ] **Переключатель режимов:** "Шаблон расписания" / "Актуальные тренировки".

- [ ] **2. Ключевые Компоненты для Реализации (в новой директории `src/features/calendar-v2/components/` или аналогичной):**
    - [ ] **`CalendarPageLayout` (или `CalendarV2Page`):** Корневой компонент страницы "Календарь v2".
        - [ ] Реализация `CalendarControlsHeader` (или секции управления), включающей:
            - [ ] Переключатель режимов ("Шаблон расписания" / "Актуальные тренировки").
            - [ ] Элементы навигации по неделям (стрелки "назад/вперед", `DatePicker`).
        - [ ] Интеграция `CalendarShell` и передача в него выбранных недели и режима.
        - [ ] Управление общим состоянием страницы (выбранная неделя, текущий режим).
    - [ ] **`CalendarShell` (Оболочка/Контейнер Календаря):**
        - [ ] Отображение сетки на 7 дней и временной шкалы на основе полученных данных (неделя, режим).
    ...
    - [ ] **`TrainingCard` (Карточка Тренировки - базовая, для отображения в слоте/стопке):**
        - [ ] Отображение основной информации: тип (название, цвет), тренер, время.
        - [ ] Цветовое кодирование по `TrainingType.color`.
